<?php
    ...
    switch($_GET["op"]){
        ...
        break;
           ...
           case "totalhardware";
            $datos=$usuario->get_usuario_totalhardware_x_id($_POST["usu_id"]);  
            if(is_array($datos)==true and count($datos)>0){
                foreach($datos as $row)
                {
                    $output["TOTAL"] = $row["TOTAL"];
           ...
           case "totalsoftware";
            $datos=$usuario->get_usuario_totalsoftware_x_id($_POST["usu_id"]);  
            if(is_array($datos)==true and count($datos)>0){
                foreach($datos as $row)
                {
                    $output["TOTAL"] = $row["TOTAL"];
           ...
           case "totalincidencia";
            $datos=$usuario->get_usuario_totalincidencia_x_id($_POST["usu_id"]);  
            if(is_array($datos)==true and count($datos)>0){
                foreach($datos as $row)
                {
                    $output["TOTAL"] = $row["TOTAL"];
           ...
           case "totalservicio";
            $datos=$usuario->get_usuario_totalservicio_x_id($_POST["usu_id"]);  
            if(is_array($datos)==true and count($datos)>0){
                foreach($datos as $row)
                {
                    $output["TOTAL"] = $row["TOTAL"];
